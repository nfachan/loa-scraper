...
async fn get_wikipedia_link(client: &Client, author: &str) -> Result<String> {
    ...
    let search_url = format!("https://en.wikipedia.org/w/api.php?action=opensearch&search={}&limit=1&format=json", 
                           urlencoding::encode(author));
    
    match client.get(&search_url)
        .header("User-Agent", "LOA-Scraper/1.0 (https://github.com/example/loa-scraper)")
        .send()
        .await {
        Ok(response) => {
...
fn parse_volumes(html: &Html) -> Result<Vec<(u32, String, String, String, String)>> {
    let book_listing_selector = Selector::parse("li.content-listing.content-listing--book").map_err(|e| anyhow!("CSS selector error: {:?}", e))?;
    let link_selector = Selector::parse("a").map_err(|e| anyhow!("CSS selector error: {:?}", e))?;
    let number_selector = Selector::parse("i.book-listing__number").map_err(|e| anyhow!("CSS selector error: {:?}", e))?;
    let title_selector = Selector::parse("b.content-listing__title").map_err(|e| anyhow!("CSS selector error: {:?}", e))?;
    ...
    for book_element in html.select(&book_listing_selector) {
        ...
        if let (Some(link), Some(number), Some(title)) = (link_element, number_element, title_element) {
            let href = link.value().attr("href").unwrap_or("");
            let volume_number = number.text().collect::<String>().trim().parse::<u32>().unwrap_or(0);
            ...
            if volume_number > 0 {
                ...
                volumes.push((volume_number, book_title, author, href.to_string(), title_text.clone()));
            }
        }
    }
...
async fn main() -> Result<()> {
    ...
    eprintln!("{} {}", "🔍".cyan(), "Scraping Library of America volumes".bright_blue().bold());
    
    let client = Client::new();
    
    // Create spinner for fetching page
    eprintln!("{} {}", "📡".yellow(), "Fetching collection page...".yellow());
    ...
    let filtered_volumes: Vec<_> = volumes_data
        .into_iter()
        .filter(|(num, _, _, _, _)| {
            *num >= start_volume && args.end.map_or(true, |end| *num <= end)
        })
        .collect();
    ...
    if filtered_volumes.is_empty() {
        eprintln!("{} {}", "⚠️".yellow(), "No volumes found in specified range".yellow());
        return Ok(());
    }
    ...
    eprintln!("{} {}", "🔗".magenta(), "Processing volumes and finding Wikipedia links...".magenta());
    
    for (i, (volume_number, title, author, loa_link, original_name)) in filtered_volumes.iter().enumerate() {
        pb.set_message(format!("Volume {}: {}", volume_number, title.chars().take(40).collect::<String>()));
        
        if i > 0 && i % 10 == 0 {
            tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
        }
        
        let wikipedia_link = match get_wikipedia_link(&client, author).await {
            Ok(link) => link,
            Err(_) => String::new(),
        };
